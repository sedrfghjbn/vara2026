"""
Django management команда для заполнения базы данных начальными данными
...
class Command(BaseCommand):
    help = 'Заполняет базу данных начальными данными для рекламного агентства'
    ...
    def handle(self, *args, **options):
        ...
        for dept_data in departments_data:
            ...
            if created:
                ...
                self.stdout.write(self.style.WARNING(f'→ Отдел уже существует: {dept.name}'))
        ...
        for pos_data in positions_data:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f'✓ Создана должность: {pos.name}'))
            else:
                self.stdout.write(self.style.WARNING(f'→ Должность уже существует: {pos.name}'))
        ...
        for emp_data in employees_data:
            ...
            if Employee.objects.filter(email=emp_data['email']).exists():
                self.stdout.write(self.style.WARNING(f'→ Сотрудник уже существует: {emp_data["email"]}'))
                continue
            ...
            self.stdout.write(self.style.SUCCESS(
                f'✓ Создан сотрудник: {employee.full_name} - {position.name if position else "Без должности"}'
            ))
        
        self.stdout.write(self.style.SUCCESS(
            f'\n✓ Заполнение завершено!'
            f'\n  Отделов: {len(departments)}'
            f'\n  Должностей: {len(positions)}'
            f'\n  Сотрудников создано: {created_count}'
        ))
